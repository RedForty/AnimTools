...
def decomposeMatricesBatch(matrices, times, rotation_order='xyz'):
    """
...
def setTransformKeysOnLayerFast(node, transform_data, times, layer=None,
    ...
    if is_matrix:
        attr_values = decomposeMatricesBatch(transform_data, times, rotation_order)
        if attrs:
...
def bakeTransformToLayerFast(source_node, target_node, start_time, end_time,
                             layer=None, sample_by=1, attrs=None):
    """
...
def cloneTransform(source=None, target=None, start_frame=None, end_frame=None,
                   layer='BaseAnimation', sample_by=1, create_locator=True):
    """
